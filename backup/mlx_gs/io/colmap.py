...
def load_colmap_dataset(path: str, images_subdir: str = "images_8"):
    """
    Load COLMAP data and convert to MLX native entities.
    """
    ...
    for info in train_cam_infos:
        ...
        camera = {
            ...
            "cx": float(cx),
            "cy": float(cy),
            "W2C": mx.array(w2c).astype(mx.float32)
        }
        mlx_cameras.append(camera)
        mlx_targets.append(mx.array(info.image).astype(mx.float32))
        
    return mx.array(xyz).astype(mx.float32), mx.array(rgb).astype(mx.float32), mlx_cameras, mlx_targets
